...
### Fixed
- An empty `TRACING_MAX_BODY_SIZE` (or `TRACING_OUTGOING_MAX_BODY_SIZE`) — e.g. a bare `TRACING_MAX_BODY_SIZE=` left over in `.env` — silently disabled truncation instead of falling back to the default. `env()` returns `''` (not the default) for a key that is present but empty, and the previous `(int) env(...)` cast turned that into `0`, which 0.5.0 redefined to mean *do not truncate*. On MySQL, where the payload columns are `text` (capped at 65535 bytes), an unbounded response over that size failed the `INSERT` with `Data too long`, and since `TracingService::persist()` logs and swallows that failure, the entire trace record was silently lost, not just its body. Both keys now fall back to the documented default of `10000` unless the env value is genuinely numeric; an explicit `0` is still honoured as the opt-out from truncation.
- Binary payloads (images, gzip streams, raw digests) could again be turned into fabricated pseudo-text, the same failure mode 0.5.0 removed charset detection to fix. `a032117` swapped the ad hoc `charset=` regex for `Symfony\Component\HttpFoundation\HeaderUtils`, which — unlike the regex — also accepts a charset value containing a comma, such as a malformed `charset="UTF-8, Windows-1251"` or `windows-1251, text/plain` produced by PSR-7's `getHeaderLine()` joining two `Content-Type` headers. `mb_convert_encoding()` treats its charset argument as a comma-separated *detection list*, not a single label, once it holds more than one name, so a multi-value charset silently re-enabled the removed detection and reintroduced the original bug. `BodyEncoding::isConvertible()` now rejects any charset that is not a single RFC-token-shaped label before it reaches `mb_convert_encoding()`; such a value is treated as undeclared, producing the `[non-UTF-8 body, N bytes]` marker for a body or leaving a parameter value as-is, same as before.
- `?date_from=` / `?date_to=` returned HTTP 500 instead of the documented 422 when the host application runs with `Carbon::useStrictMode(false)`. `Carbon::rawCreateFromFormat()` is declared `?static` and only throws `InvalidFormatException` in strict mode (Carbon's default); with strict mode off it returns `null` instead, which the subsequent round-trip check then called `format()` on. The loop over `DATE_FORMATS` now treats a `null` result the same as a caught exception and tries the next format.
- `?date_to=` with minute precision (`Y-m-d H:i`) excluded its own last minute instead of including it, asymmetrically with a bare date (`Y-m-d`), which was already rounded up to end-of-day. `?date_to=2026-01-01 10:30` silently dropped every record between `10:30:00` and `10:30:59`. The upper bound is now rounded up to the end of whichever unit the matched format leaves unspecified: end-of-day for `Y-m-d`, end-of-minute for `Y-m-d H:i`, and unchanged for the fully-specified `Y-m-d H:i:s`.
